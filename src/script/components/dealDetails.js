...
export default class DealDetails extends Component {
    render({ deal, showNext, showPrevious }) {
        ...
        const links = [
            {
                label: 'Tell a Friend',
                url: `https://slickdeals.net/forums/send_to_friend.php?threadid=${deal.threadId}&couponid=0&url=${encodedDealUrl}`,
        ...
        return (
            <article class="deal-details" id={deal.id}>
                ...
                <section class="details-text" dangerouslySetInnerHTML={{ __html: deal.text }} />
            </article>
        );
    }
}
